...
(function () {
    ...
    if (s && s.dataset.goatcounterSettings) {
        ...
        for (var k in set)
            if (
                ['no_onload', 'no_events', 'allow_local', 'allow_frame', 'path', 'title', 'referrer', 'event'].indexOf(
                    k
                ) > -1
            )
    ...
    goatcounter.filter = function () {
        ...
        if (
            ...
            location.hostname.match(/(localhost$|^127\.|^10\.|^172\.(1[6-9]|2[0-9]|3[0-1])\.|^192\.168\.|^0\.0\.0\.0$)/)
        )
            return 'localhost';
